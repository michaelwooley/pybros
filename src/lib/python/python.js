...
(function () {
	...
	loadPackage({
		files: [
			{ filename: '/usr/local/lib/python311.zip', start: 0, end: 3192755 },
			{ filename: '/usr/local/lib/python3.11/os.py', start: 3192755, end: 3232220 },
			{ filename: '/usr/local/lib/python3.11/lib-dynload/.empty', start: 3232220, end: 3232220 }
		],
		remote_package_size: 3232220,
		package_uuid: '190c9ec9-0fcd-421b-87ae-d3f13ce4b4f7'
	});
})();
...
if (ENVIRONMENT_IS_NODE) {
	...
	// if (ENVIRONMENT_IS_WORKER) { scriptDirectory = require('path').dirname(scriptDirectory) + '/'; } else { scriptDirectory = __dirname + '/'; } requireNodeFS = () => { if (!nodePath) { fs = require('fs'); nodePath = require('path'); } }; read_ = function shell_read(filename, binary) { requireNodeFS(); filename = nodePath['normalize'](filename); return fs.readFileSync(filename, binary ? undefined : 'utf8'); }; readBinary = (filename) => { var ret = read_(filename, true); if (!ret.buffer) { ret = new Uint8Array(ret); } return ret; }; readAsync = (filename, onload, onerror) => { requireNodeFS(); filename = nodePath['normalize'](filename); fs.readFile(filename, function (err, data) { if (err) onerror(err); else onload(data.buffer); }); }; if (process['argv'].length > 1) { thisProgram = process['argv'][1].replace(/\\/g, '/'); } arguments_ = process['argv'].slice(2); if (typeof module != 'undefined') { module['exports'] = Module; } process['on']('uncaughtException', function (ex) { if (!(ex instanceof ExitStatus)) { throw ex; } }); process['on']('unhandledRejection', function (reason) { throw reason; }); quit_ = (status, toThrow) => { if (keepRuntimeAlive()) { process['exitCode'] = status; throw toThrow; } logExceptionOnExit(toThrow); process['exit'](status); }; Module['inspect'] = function () { return '[Emscripten Module object]'; };
		...
		FS.registerDevice(dev, {
			...
			read: (stream, buffer, offset, length, pos) => {
				...
				for (var i = 0; i < length; i++) {
					...
					try {
						result = input();
						console.log('Calling read on ', name, result, stream, buffer, offset, length, pos);
					} catch (e) {
						throw new FS.ErrnoError(29);
					}
					if (result === undefined && bytesRead === 0) {
...
var _Py_EMSCRIPTEN_SIGNAL_HANDLING = (Module['_Py_EMSCRIPTEN_SIGNAL_HANDLING'] = 3377184);
...
function run(args) {
	...
	function doRun() {
		...
		if (shouldRunNow) {
			console.log('Running should run now');
...
// module.exports = { Module, callMain };
